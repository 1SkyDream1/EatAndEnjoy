(() => {
    ...
    (() => {
        ...
        const cart = document.querySelector(".catalog__cart span");
        const infoPrice = document.querySelector(".payment__price span");
        const infoProducts = document.querySelector(".payment__products");
        if (cartButtons) cartButtons.forEach((button => {
            button.addEventListener("click", (function(e) {
                e.preventDefault();
        ...
        function addProduct(info) {
            ...
            infoPrice.innerHTML = `${Number(infoPrice.innerHTML) + Number(price)}`;
            console.log(infoProducts);
            if (infoProducts.innerHTML == "Пока что пусто") {
                infoProducts.innerHTML = "";
                infoProducts.innerHTML = `${name} (${price}р)`;
            } else infoProducts.innerHTML = `${infoProducts.innerHTML}</br> ${name} (${price}р)`;
        }
        const resetButton = document.querySelector(".payment__reset");
        resetButton.addEventListener("click", (function(e) {
            cart.innerHTML = "0";
            infoPrice.innerHTML = "0";
            infoProducts.innerHTML = "Пока что пусто";
        }));
        window["FLS"] = true;
        isWebp();
        rippleEffect();
